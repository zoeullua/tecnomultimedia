...
String[] textos_b = new String [14];
int estado = 0;
int haybotones = 0; // esta var va a ser 1 cuando haya botones de opcion, 0 cuando no
int numbotonA = 0;  // aca se van a cargar los textos de los botones
int numbotonB = 1; // aca se van a cargar los textos de los botones
int avanzo = 1;  // esta var va a servir de traba, para que no avance solo de pantallas al presionar los botones
...
textos[13]= "CREDITOS:\n Juego programado y escrito por: Zoe Ullua. \n Obra original: La bella y la bestia, escrita por la autora Jeanne-Marie Leprince de Beaumont. \n Las imagenes utilizadas son de la película de Disney.";
...
void draw(){
   ...
   if (haybotones == 1) {
     botones(textos_b[numbotonA], textos_b[numbotonB]); // programo los botones de opciones
   }
   // programo los botones de reinicio comienzo y creditos
   if (estado == 0){
...
void mousePressed(){
  ...
  if (tocabotonA(20, 270, 320, 420)){ // boton A
   ...
   if (estado == 11){
     haybotones =0;
     avanzo=0;
    if (mouseX> 230 && mouseX< 380 && mouseY > 10 && mouseY< 60){ // boton CREDITOS
      println("cambia a 13");
      estado = 13;
